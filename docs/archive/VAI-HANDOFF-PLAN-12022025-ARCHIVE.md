...
- D. DevOps, CI/CD, and environments (Vercel, Cloud Run, Firestore)
...
- Confirm which Git branch currently matches what is deployed or considered the "baseline" (ExecutionPhaseSprint) and which branch contains Tailwind + TypeScript work (TailwindTSX).
...
### B1. Lock down API endpoints with consistent auth
...
- Confirm that public routes like survey submission remain open but still validate inputs and rate-limit if needed.
...
### B3. Data retention and PII strategy (180-day soft policy)
...
- Agree on a small but meaningful testing target that can be achieved before handoff, such as:
    - A handful of frontend component tests for critical flows (survey form, login, dashboard routing)
    - A few backend tests for key routes (submit-survey, verify-token, get-survey-responses, generate-report)
    - One or two end-to-end (E2E) smoke tests that run the entire flow from survey submission to dashboard viewing.
- Decide on the primary tools (for example, Vitest + React Testing Library for frontend, pytest for backend, Playwright or Cypress for E2E).
...
- Clearly list features and improvements that are intentionally deferred beyond this handoff, such as:
    ...
    - Multilingual survey support and more advanced accessibility.
- For each deferred item, add one or two sentences explaining why it is important and how a future team might approach it.
